...
from get_yolo_output import get_detection
...
parser.add_argument('--train_itr', type = int, default = 2)
parser.add_argument('--batch_size', type = int, default = 6)
parser.add_argument('--inference_batch', type = int, default = 10)
...
parser.add_argument('--policy_type', type = str, default = 'greedy')
...
parser.add_argument('--save_dir', type = str, default = 'log/dis_clip')
parser.add_argument('--train_image_path', type = str, default = 'VOCdevkit/VOC2007/')
parser.add_argument('--labels', type =  str, default = 'resized_labels.csv')
...
img_label = get_label(args.labels)
train_image_path = args.train_image_path + str('resized_img/')
...
def main():
    ...
    for epoch in tqdm(range(args.epochs)): 
        ...
        train_image_list = random.sample(image_list, args.steps)

        Reward = []
        Img_path_list = []
        step = 0

        greed_range = np.arange(start = 0.1, stop = 0.05, step = -0.0001)
        if epoch<500:
            greed = greed_range[epoch]
        else:
            greed = 0.05
            
        for i in range(int(args.steps/args.inference_batch)):
            Orig_img = torch.zeros(args.inference_batch,*(3,*(args.image_dim)))
            Noisey_img = torch.zeros(args.inference_batch,*(3,*(args.image_dim)))
            Refined_img = torch.zeros(args.inference_batch,*(3,*(args.image_dim)))
            Image_name = []

            for j in range(args.inference_batch):
                img_name = train_image_list[step]
...
####################____Preprocess_Image_and_Add_Noise____####################
                ...
                orig_img_tensor = prep_image(orig_img_array, args)

                if args.action_type=='discrete':
                    noisey_img, noise_scale = synthetic_change_dis(args, img, flag = 1)
                if args.action_type == 'continuous':
                    noisey_img, noise_scale = synthetic_change_cont(args, img, flag =1)
                noisey_img_array = np.array(noisey_img)
                noisey_img_tensor = prep_image(noisey_img_array, args)

####################____Refine_Image____####################

                state = noisey_img_tensor.unsqueeze(0)
                with torch.no_grad():
                    action_, action, action_prob, value = policy(state, greed)

                refined_img = change_brightness(noisey_img, action_)
                refined_img_array = np.array(refined_img)
                refined_img_tensor = prep_image(refined_img_array,args)
 
                Orig_img[j,...] = orig_img_tensor
                Noisey_img[j,...] = noisey_img_tensor
                Refined_img[j,...] = refined_img_tensor

                memory.update_buffer(step, noise_scale, action, action_prob, value)
                step +=1

####################____Get_Predication____####################

            Orig_detect = detector.detector(Orig_img, Image_name)
            Noisey_detect = detector.detector(Noisey_img, Image_name)
            Refined_detect = detector.detector(Refined_img, Image_name)

####################____Compute_Reward____####################

            for key in Orig_detect.keys():
                original_detect = Orig_detect[key]
                noisey_detect = Noisey_detect[key]
                refined_detect = Refined_detect[key]

                ground_truth_df = img_label[img_label['filename'] == key]
                ground_truth_arr = []
                for k in range(len(ground_truth_df)):
                    ground_truth_arr.append(ground_truth_df.iloc[k][1:])
                ground_truth_arr = np.array(ground_truth_arr)

                original_pred_array = pred_array(original_detect, classes, voc_classes)
...
####################____Train_and_Print_Summary____####################
        ...
        Reward_buff = torch.Tensor(Reward).to(args.device)
        Reward_buffer = torch.unsqueeze(Reward_buff,1)

        for i in range(args.train_itr):
            batch_idx = np.random.randint(0,args.steps,args.batch_size)
            noise_scale, old_action, old_action_prob, old_reward, old_value = memory.fetch_data(batch_idx, Reward_buffer)
            ...
            *_, new_log_prob, new_value = policy(b_image, greed, b_action)
